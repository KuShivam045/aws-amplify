...
const ProposalDetails = (props) => {
  ...
  const acceptRejectApi = (status) => {
    ...
    fetch(
      process.env.BASE_URL + "/api/freelancerapp/rozgaarapi/AcceptRejectproposal",
      requestOptions
    )
  ...
  const requirementPayment = (fProposalID, requirmentID) => {
    ...
    fetch(
      process.env.BASE_URL + "/api/rozgaarapi/ClientFreelancerPaymentDepositInitiate",
      ...
      .then((result) => {
        ...
        if (result.status === "SUCCESS" && result.status_code === 200) {
          ...
          router.push("/razorpaygateway/" + result.data.PGOrderId + "/"+ requirmentID)
          // { state: { RequirementID: requirmentID } });
        }
      })
      .catch((error) => {
      ...
      return (
        <CustomModalTheme width={"fullWidth"} onClose={props.onClose}>
          <div>
            ...
            <div className={classes.heading}>Description
              <p className="text-prose prose leading-6 font-normal">
...
export const encodeAndSendToChat = (senderId, recieverId) => {
  ...
  window.open(`${process.env.CHAT_URL}/chat?token=${token}`)
}
